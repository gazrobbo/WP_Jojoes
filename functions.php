...
function fump_setup() {
	 ...
	 * Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/Fump
	 * If you're building a theme based on Ignition, and you downloaded this from github, use a find and replace
	 * to change 'fump' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'fump' );
...
add_action( 'after_setup_theme', 'fump_setup' );
...
if ( ! function_exists( 'ign_widgets_init' ) ) {
	function ign_widgets_init() {
		register_sidebar( array(
			'name'          => __( 'Sidebar', 'fump' ),
			'id'            => 'sidebar-1',
			'description'   => __( 'Add widgets here to appear in your sidebar.', 'fump' ),
 ...
 */
function fump_scripts() {

	//load regular versions if script debug is set to true in wp-config file.
	$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';

	// Add google fonts
	wp_enqueue_style( 'fump-fonts', ign_google_fonts_url(), array(), wp_get_theme()->get( 'Version' ) );

	// Theme stylesheet. Will get this stylesheet or a child themes stylesheet.
	wp_enqueue_style( 'fump-style', get_stylesheet_uri(), '', wp_get_theme()->get( 'Version' ) );

	//Sass compiles styles. Will get child's theme version if found instead. Child theme should import with sass.
	wp_enqueue_style( 'fump-sass-styles', get_theme_file_uri( '/dist/main' . $suffix . '.css' ), '', wp_get_theme()->get( 'Version' ) );
	...
	wp_enqueue_script( 'fump-custom-js', get_template_directory_uri() . '/dist/custom' . $suffix . '.js', array( 'jquery' ),
		wp_get_theme()->get( 'Version' ), true );

	//AJAX ready for .custom.js files
	wp_localize_script( 'fump-custom-js', 'frontEndAjax', array(
	...
	wp_localize_script( 'fump-custom-js', 'icons', array(
...
add_action( 'wp_enqueue_scripts', 'fump_scripts' );
...
function footer_styles() {
	//load regular versions if script debug is set to true in wp-config file.
	$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
	wp_enqueue_style( 'fump-admin-styles', get_theme_file_uri( '/dist/admin' . $suffix . '.css' ), '', wp_get_theme()->get( 'Version' ) );
}
 ...
 */
function fump_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">' . "\n", get_bloginfo( 'pingback_url' ) );
	}
}

add_action( 'wp_head', 'fump_pingback_header' );
