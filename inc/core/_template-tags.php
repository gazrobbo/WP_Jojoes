...
if ( ! function_exists( 'ign_posted_by' ) ) {
	function ign_posted_by() {
		...
		$author_name        = sprintf( __( '%s by %s', 'fump' ), '<a href="' . $author_link . '" class="author-name"><span class="byline">', '</span>' . get_the_author() . '</a>' );
	...
	function ign_get_term_links( $taxonomy = 'category', $get_all = false, $id = 0 ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) :
			...
			if ( ! $get_all ) {
				...
				$term_links_output = join( '<span class="delim">' . __( ', ', 'fump' ) . '</span>', $term_links );
			}
	...
	function ign_edit_link( $id = null, $class = '', $text = 'Edit' ) {
		...
		edit_post_link(
			...
			/* translators: %s: Name of current post */
				__( '%s<span class="screen-reader-text"> "%s"</span>', 'fump' ),
				$text, get_the_title()
			),
...
function ign_loop( $file_prefix = 'content' ) {
	...
	//if the post type has a folder in Fump/template-parts we use that, else we default to post folder
